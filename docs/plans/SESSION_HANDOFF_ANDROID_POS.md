...
For the event-reporting follow-up, 111 focused tests passed. After rebasing onto `main@438bb81`, the full non-browser suite passed serially with 396 files passed, 1 skipped, 4,319 tests passed and 13 opt-in tests skipped; Electron smoke then passed separately with 13/13 tests. Renderer/main/Windows build, Android web build, production-readiness policy, build-only CI policy and boundary checks passed (162 reachable source files / 5 bundle files). These are not native Android, real backend or printer acceptance. Inspect fresh PR CI before merge. Old APKs do not represent this source. Do not publish a release tag, auto-update artifact or production APK just because a source PR merged.
