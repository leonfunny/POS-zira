...
Status: implementation in stages; guarded client opt-in is implemented, but production remains unavailable because the backend capability is not landed/deployed and release gates remain open.

## Stage 6 verified result — capability negotiation and guarded V1 coordinator

- A new refund probes capabilities inside the frozen server/salon/user/token context. Only a direct own numeric `refundEventVersion: 1` selects V1; absent, inherited, wrapped or malformed values retain the legacy protocol. Existing PREPARED/UNKNOWN/CONFIRMED attempts never re-probe or rewrite their frozen protocol, request ID or serialized payload. Once an order has canonical events it cannot downgrade to legacy.
- Before V1 dispatch, the coordinator durably retains the device UUID and validates UUID identities, the sole current open sale shift, exact server/salon/machine/backend-shift binding, ordinary fully settled order money, unique backend mapping, complete canonical history/audit, original and remaining tender capacities, and immutable local projection. The frozen request is independently revalidated before every POST/replay. Canonical response validation then inserts the event, updates the order and confirms the journal in one transaction, followed by the existing latched durable flush. A failed final flush restarts from durable UNKNOWN rather than issuing a second refund.
- Same-sale-shift, single-tender, supported CASH/CARD/BLIK/BANK_TRANSFER, no tip/discount/fee, exact-line and OWNER/MANAGER gates remain. Split, cross-shift and OTHER are not enabled. Capability lookup failure does not dispatch a refund. Read-only detail can continue for a coherently converted order, while malformed/orphan event evidence stays blocked.
- Local evidence after independent-audit fixes: 193 focused refund tests passed. The full serial suite passed 398 files with 1 opt-in browser file skipped (4,378 tests passed, 13 skipped), including Electron smoke 13/13. Full Windows build, Android web build, boundary checks (162 source files / 5 bundles), production-readiness policy and build-only CI policy passed. The production gate deliberately remains NO-GO with 22 recorded blockers.
- This is conditional client wiring, not a production activation or acceptance. The separate backend V1 worktree is still uncommitted/undeployed, so deployed servers do not provide verified capability evidence for this source. No native asset sync, APK/installer publication, device install, real refund, backend deployment, tag or production restart is included. Next: recoverable remote-close acknowledgement/outbox, separate backend landing, and native restart/offline/tenant/payment/refund/printer acceptance.
...
- At this stage V1 HTTP dispatch was still OFF. Stage 6 above adds guarded conditional wiring without deploying the required backend capability. Cross-shift/split/OTHER gate removal, remote-close outbox, native asset sync, APK, device install, real refund, backend deployment and production release remain excluded.
