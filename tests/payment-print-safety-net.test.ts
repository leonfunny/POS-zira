...
describe('PaymentController — sale completes despite print/drawer failure (G2)', () => {
  ...
  it('treats a configured driver with a failed reconnect as no local outbox route', async () => {
    const printer = makeFakePrinter({ connected: false, connectSucceeds: false });
    const ctl = buildController(printer);

    await expect(ctl.isLocalReceiptPrinterReadyForOutbox()).resolves.toBe(false);
    expect(printer.connect).toHaveBeenCalledTimes(1);
    expect(printer.printReceipt).not.toHaveBeenCalled();
  });

  it('keeps the local outbox route when reconnect succeeds', async () => {
    const printer = makeFakePrinter({ connected: false, connectSucceeds: true });
    const ctl = buildController(printer);

    await expect(ctl.isLocalReceiptPrinterReadyForOutbox()).resolves.toBe(true);
    expect(printer.connect).toHaveBeenCalledTimes(1);
  });

  it('never falls through to shared printing after a selected local outbox route disappears', async () => {
    const printer = makeFakePrinter({ connected: false, connectSucceeds: false });
    const sharedReceiptPrinter = vi.fn(async () => ({
      handled: true,
      printed: false,
      stillPrinting: true,
      printerId: 'remote-printer',
      jobId: 'remote-job',
    }));
    const ctl = buildController(printer, sharedReceiptPrinter);

    await expect(ctl.printReceipt(
      'order-1',
      undefined,
    ...
    const drawerResult = await ctl.printReceiptAndOpenDrawer(
      'order-1',
      undefined,
      { localOnly: true },
    );

    expect(drawerResult).toMatchObject({
  ...
  it('opens the drawer separately only when a combined failure is explicitly SAFE_BEFORE_PRINT', async () => {
    const printer = makeFakePrinter({}) as ReturnType<typeof makeFakePrinter> & {
      printReceiptWithDrawer: ReturnType<typeof vi.fn>;
    };
    const error = Object.assign(new Error('worker rejected before WritePrinter'), {
      failureClass: 'SAFE_BEFORE_PRINT' as const,
    });
    printer.printReceiptWithDrawer = vi.fn(async () => {
      throw error;
    });
    const ctl = buildController(printer);
    ...
    expect(result.receiptPrinted).toBe(false);
    expect(result.drawerOpened).toBe(true);
    expect(printer.printReceiptWithDrawer).toHaveBeenCalledTimes(1);
  ...
  it.each([
    {
      label: 'an explicit uncertain drawer failure',
      drawerError: Object.assign(new Error('drawer WritePrinter outcome unknown'), {
        failureClass: 'UNCERTAIN_AFTER_PRINT' as const,
      }),
      expectedFailureClass: 'UNCERTAIN_AFTER_PRINT',
    },
    {
      label: 'an unclassified drawer driver failure',
      drawerError: new Error('drawer driver outcome unknown'),
      expectedFailureClass: 'UNCERTAIN_AFTER_PRINT',
    },
    {
      label: 'an explicit safe drawer failure',
      drawerError: Object.assign(new Error('drawer rejected before StartDoc'), {
        failureClass: 'SAFE_BEFORE_PRINT' as const,
      }),
      expectedFailureClass: 'SAFE_BEFORE_PRINT',
    },
  ] as const)(
    'classifies the combined outcome correctly after $label',
    async ({ drawerError, expectedFailureClass }) => {
      const printer = makeFakePrinter({}) as ReturnType<typeof makeFakePrinter> & {
        printReceiptWithDrawer: ReturnType<typeof vi.fn>;
      };
      const receiptError = Object.assign(new Error('receipt rejected before StartDoc'), {
        failureClass: 'SAFE_BEFORE_PRINT' as const,
      });
      printer.printReceiptWithDrawer = vi.fn(async () => {
        throw receiptError;
      });
      printer.openDrawer = vi.fn(async () => {
        throw drawerError;
      });
      const ctl = buildController(printer);
      ...
      expect(result).toMatchObject({
        ...
        error: expect.stringContaining(drawerError.message),
      });
      expect(printer.printReceiptWithDrawer).toHaveBeenCalledTimes(1);
      expect(printer.openDrawer).toHaveBeenCalledTimes(1);
    },
  ...
  it.each([
    Object.assign(new Error('WritePrinter outcome unknown'), {
      failureClass: 'UNCERTAIN_AFTER_PRINT' as const,
    }),
    new Error('unclassified combined driver failure'),
  ])('does not send a standalone drawer pulse after an uncertain combined failure', async (error) => {
    const printer = makeFakePrinter({}) as ReturnType<typeof makeFakePrinter> & {
      printReceiptWithDrawer: ReturnType<typeof vi.fn>;
    };
    printer.printReceiptWithDrawer = vi.fn(async () => {
