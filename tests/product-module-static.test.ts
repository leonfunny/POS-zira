...
describe('Product module implementation contract', () => {
  ...
  it('enables category management only through backend product-admin IPC', () => {
    ...
    expect(categoryManager).toContain("products.category.displaySettings");
    expect(categoryManager).toContain('type="color"');
    ...
    expect(categoryManager).toContain('const [savingCategoryIds, setSavingCategoryIds]');
    expect(categoryManager).toContain('disabled={creating || mutationBusy || (!categoriesLoaded && !createAttemptLocked)}');
    expect(categoryManager).toContain("products.category.colorHex");
    expect(categoryManager).not.toContain('category.updatedAt, category.version');
    expect(categoryManager).toContain('const mutationBusy = loading || creating || ordering');
    expect(categoryManager).toContain('disabled={!canUpdateCategory || ordering || busy}');
    expect(categoryManager).toContain('aria-invalid={!validColor}');
    ...
    expect(categoryManager).toContain('disabled={mutationBusy || !categoriesLoaded || createAttemptLocked}');
    expect(categoryManager).toContain("products.category.createOutcomeUncertain");
    expect(categoryManager).toContain('const refreshed = await loadCategories();');
    expect(categoryManager).toContain('if (refreshed) {');
    ...
    expect(categoryManager).toContain('onSaved: () => Promise<boolean>;');
    expect(categoryManager).toContain('const refreshed = await onSaved();');
    expect(categoryManager).toContain('if (!canReorderCategory || !categoriesLoaded || mutationBusy) return;');
    expect(categoryManager).toContain('mutationDisabled={mutationBusy || !categoriesLoaded}');
