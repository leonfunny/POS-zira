...
describe('Product module implementation contract', () => {
  ...
  it('enables product edit and stop-selling only through backend product-admin IPC', () => {
    ...
    expect(editForm).toContain('products.edit.trackInventoryHint');
    expect(editForm).toContain('products.edit.inventoryTrackingZeroRequired');
    expect(editForm).toContain('products.edit.inventoryTrackingUnavailable');
    expect(moduleSource).toContain('canChangeInventoryMode={adminCapabilities?.canChangeInventoryMode === true}');
    ...
    for (const locale of ['en', 'vi', 'pl'] as const) {
      ...
      expect(translationBlock(locale)).toContain("'products.edit.trackInventoryHint':");
      expect(translationBlock(locale)).toContain("'products.edit.inventoryTrackingZeroRequired':");
      expect(translationBlock(locale)).toContain("'products.edit.inventoryTrackingUnavailable':");
    }
  ...
  it('enables category management only through backend product-admin IPC', () => {
    ...
    expect(moduleSource).toContain('adminCapabilities?.canCreateCategory === true');
    expect(moduleSource).toContain('adminCapabilities?.canUpdateCategory === true');
    expect(moduleSource).toContain('adminCapabilities?.canDeleteCategory === true');
    expect(moduleSource).toContain('adminCapabilities?.canReplaceCategoryImage === true');
    ...
    expect(moduleSource).toContain('localProductCounts={localProductCountsByCategory}');
    expect(categoryManager).toContain('localCategoryCount');
    expect(categoryManager).toContain('localProductCounts');
    ...
    expect(categoryManager).toContain('window.electronAPI.pos.productAdmin.updateCategoryOrder');
    expect(categoryManager).toContain('window.electronAPI.pos.productAdmin.uploadCategoryImage');
    expect(categoryManager).toContain('window.electronAPI.pos.productAdmin.deleteCategory');
    expect(moduleSource).toContain('canReorderCategory={adminCapabilities?.canReorderCategory === true && adminCapabilities?.supportsCategoryBatchUpdate === true}');
    expect(moduleSource).toContain('canDeleteCategory={adminCapabilities?.canDeleteCategory === true}');
    expect(moduleSource).toContain('canReplaceCategoryImage={adminCapabilities?.canReplaceCategoryImage === true}');
    expect(moduleSource).toContain('supportsCategoryImageUpload={adminCapabilities?.supportsCategoryImageUpload === true}');
    expect(categoryManager).toContain('canReorderCategory: boolean');
    expect(categoryManager).toContain('canDeleteCategory: boolean');
    expect(categoryManager).toContain('canReplaceCategoryImage: boolean');
    expect(categoryManager).toContain('supportsCategoryImageUpload: boolean');
    expect(categoryManager).toContain('!canReorderCategory');
    expect(categoryManager).toContain("products.category.image");
    expect(categoryManager).toContain('accept="image/jpeg,image/png,image/webp"');
    expect(categoryManager).toContain('MAX_CATEGORY_IMAGE_BYTES');
    expect(categoryManager).toContain('CategoryImagePicker');
    expect(categoryManager).toContain('DeleteCategoryDialog');
    expect(categoryManager).toContain('ChevronUp');
    expect(categoryManager).toContain('ChevronDown');
    expect(categoryManager).toContain('nextCategorySortOrder(categories)');
    expect(categoryManager).not.toContain('inputMode="numeric"');
    expect(categoryManager).not.toContain('parseSortOrder');
    expect(categoryManager).not.toContain('CategoryAppearanceFields');
    expect(categoryManager).not.toContain('isValidCategoryColor');
    expect(categoryManager).not.toContain('isValidCategoryIcon');
    expect(categoryManager).not.toContain('type="color"');
    expect(categoryManager).toContain('const [categoriesLoaded, setCategoriesLoaded]');
    expect(categoryManager).toContain('const [savingEditor, setSavingEditor]');
    expect(categoryManager).toContain('const [deleteTarget, setDeleteTarget]');
    expect(categoryManager).toContain('const mutationBusy = loading || savingEditor || ordering || deleting');
    expect(categoryManager).toContain("role={message.ok ? 'status' : 'alert'}");
    expect(categoryManager).toContain('createStableMutationKeyStore');
    expect(categoryManager).toContain('const createAttemptRef');
    expect(categoryManager).toContain('draftFingerprint');
    expect(categoryManager).toContain('createAttemptRef.current = attempt');
    expect(categoryManager).toContain('createAttemptRef.current = null');
    expect(categoryManager).toContain('const [createAttemptLocked, setCreateAttemptLocked]');
    expect(categoryManager).toContain("products.category.createOutcomeUncertain");
    expect(categoryManager).toContain('let createCommitted = false;');
    expect(categoryManager).toContain('if (createCommitted) {');
    expect(categoryManager).toContain('setCreateAttemptLocked(true);');
    expect(categoryManager).toContain(
      'mutationBusy || editorMode !== null || deleteTarget !== null || createAttemptLocked',
    );
    expect(categoryManager).toContain('busy={parentModalBusy}');
    expect(categoryManager).toContain('shouldKeepCategoryCreateAttemptLocked');
    expect(categoryManager).toContain('const previousCategories = categories;');
    expect(categoryManager).toContain('const refreshedAfterFailure = await loadCategories();');
    expect(categoryManager).toContain('if (!refreshedAfterFailure) setCategories(previousCategories);');
    expect(categoryManager).toContain("products.category.savedRefreshFailed");
    expect(categoryManager).toContain("products.category.createdRefreshFailed");
    expect(categoryManager).toContain("products.category.deletedRefreshFailed");
    expect(categoryManager).toContain("products.category.alreadyDeleted");
    expect(categoryManager).toContain("result?.status === 404");
    expect(categoryManager).toContain("result?.status === 409");
    expect(categoryManager).toContain("products.category.orderRefreshFailed");
    expect(categoryManager).toContain('if (!canReorderCategory || !categoriesLoaded || mutationBusy) return;');
    ...
    expect(categoryManager).toContain('expectedUpdatedAt: category.updatedAt,');
    expect(categoryManager).toContain('productCount={category.productCount ?? localProductCounts[category.id] ?? 0}');
    expect(categoryGrid).toContain('category.image_url');
    expect(categoryGrid).toContain('object-cover');
