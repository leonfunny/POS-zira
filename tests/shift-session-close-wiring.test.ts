...
describe('POS shift close session identity wiring', () => {
  it('routes delayed manual and EOD close paths through the identity guard', () => {
    ...
    expect(source.match(/this\.closeSessionIfShiftMatches\(\[data\.shiftId\]\);/g)).toHaveLength(3);
    expect(source).toContain('this.closeSessionIfShiftMatches(openShifts.map((shift) => shift.id));');
  });
});
