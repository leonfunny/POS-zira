import { mkdirSync, copyFileSync, rmSync, existsSync, createWriteStream } from 'fs';
...
export class AdVideoStore {
  ...
  async addUpload(originalName: string, stream: Readable): Promise<AddedVideo> {
    ...
    const id = `ad_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
    const filename = `${id}${ext}`;
    await pipeline(stream, createWriteStream(join(this.baseDir, filename), { flags: 'wx' }));
    return type === 'image' ? { id, filename, type, durationMs: 7000 } : { id, filename, type };
  }
