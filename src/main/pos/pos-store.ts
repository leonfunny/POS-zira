...
// === State interfaces ===
//
// MOVED to src/shared/pos/pos-state.ts so the Android shim can build on the
// SAME definitions instead of re-declaring them (a re-declared cart shape is
// how the billiard POS-handoff drift happened). Re-exported here so every
// existing `from './pos-store'` import keeps working untouched.
