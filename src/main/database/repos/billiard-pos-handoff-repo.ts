...
export const billiardPosHandoffRepo = {
  ...
  markState(checkoutId: string, state: BilliardLocalHandoffState): boolean {
    ...
    return billiardPosHandoffRepo.get(checkoutId)?.state === state;
  },
  ...
  markTenderCommitting(checkoutId: string): boolean {
    ...
    return billiardPosHandoffRepo.get(checkoutId)?.state === 'POS_TENDER_COMMITTING';
  },
  ...
  markTenderUncertain(checkoutId: string): boolean {
    ...
    return billiardPosHandoffRepo.get(checkoutId)?.state === 'POS_TENDER_UNCERTAIN';
  },
  ...
  rollbackTenderBeforeCharge(checkoutId: string, noChargeConfirmed: boolean): boolean {
    ...
    return billiardPosHandoffRepo.get(checkoutId)?.state === 'POS_PAYMENT_OPEN';
  },
  ...
  markTenderUncertainAfterRollbackFailure(checkoutId: string): boolean {
    ...
    if (current.state === 'POS_TENDER_COMMITTING') {
      return billiardPosHandoffRepo.markTenderUncertain(checkoutId);
    }
    if (current.state !== 'POS_PAYMENT_OPEN') return false;
    database.run(
      `UPDATE pos_billiard_handoffs
  ...
  rollbackNoPaymentResolution(
    ...
    const rolledBack = billiardPosHandoffRepo.get(checkoutId);
  ...
  setCorrectionIntent(orderId: string, intent: BilliardCorrectionIntent | null): boolean {
    const current = billiardPosHandoffRepo.getByOrderId(orderId);
    if (!current) return false;
    database.run(
      `UPDATE pos_billiard_handoffs
    ...
    const updated = billiardPosHandoffRepo.getByOrderId(orderId);
    return intent
