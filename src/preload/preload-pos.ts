...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  pos: {
    ...
    billiardCheckout: {
      ...
      beginTender: (checkoutId: string, paymentPreflightToken: string) => ipcRenderer.invoke('pos:billiard:begin-tender', checkoutId, paymentPreflightToken),
      beginRestoredTender: (holdId: string, paymentPreflightToken: string) => ipcRenderer.invoke('pos:restored-cart:begin-tender', holdId, paymentPreflightToken),
    ...
    orders: {
      create: (order: any, items: any[], options?: { queueInitialReceipt?: boolean }) =>
        ipcRenderer.invoke('pos:orders:create', order, items, options),
    ...
    payment: {
      ...
      preflight: (orderId: string) => ipcRenderer.invoke('pos:payment:preflight', orderId),
    ...
    fabricTagArtworks: createFabricTagArtworksBridge((channel, ...args) =>
      ipcRenderer.invoke(channel, ...args),
    ),
    masterCatalog: {
      ...
      scanCreate: (payload: { ean: string; name?: string; attachVariantId?: string; purchasePrice?: number; retailPrice?: number; stockQty?: number; taxRate?: number; warehouseId?: string; categoryId?: string | null; createIfMiss?: boolean; idempotencyKey?: string }) =>
        ipcRenderer.invoke('pos:master-catalog:scan-create', payload),
      importDraft: (payload: { ean: string; retailPriceGrosze?: number; categoryId?: string; stockQty?: number }) =>
