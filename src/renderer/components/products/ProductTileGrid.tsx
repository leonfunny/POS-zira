...
const PRODUCT_TILE_PAGE_SIZE = 300;
      ...
      <div className="min-h-0 flex-1 overflow-y-auto pr-1">
        {visibleProducts.length > 0 ? (
          <>
            ...
            {truncated ? (
              <div className="mt-4 flex flex-wrap items-center justify-center gap-3 rounded-md border border-sky-200 bg-sky-50 px-3 py-3 text-sm text-sky-800">
                <span>
                  {tOr(t, 'products.renderLimit', 'Showing')} {visibleProducts.length}/{products.length}
                </span>
                <button
                  type="button"
                  onClick={() => setVisibleLimit((current) => current + PRODUCT_TILE_PAGE_SIZE)}
                  className="inline-flex h-11 items-center rounded-md border border-sky-300 bg-white px-4 font-semibold text-sky-900 hover:bg-sky-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-500"
                >
