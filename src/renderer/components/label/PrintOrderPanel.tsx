...
const COPY: Record<string, Copy> = {
  ...
  tr: {
    title: 'Baskı siparişi',
    subtitle: 'Müşterinin A4 sayfasını girin, sonra bir kerede yazdırın',
    customer: 'Müşteri',
    styleName: 'Model adı',
    styleCode: 'Model kodu',
    materials: 'Bileşim',
    materialsHint: 'Bir malzemeye dokunun ve yüzdeyi yazın',
    care: 'Bakım sembolleri',
    careGroup: { wash: 'Yıkama', bleach: 'Ağartma', tumble: 'Kurutma makinesi', natural: 'Doğal kurutma', iron: 'Ütüleme', dryclean: 'Kuru temizleme', wetclean: 'Islak temizleme' },
    careText: 'Ek satırlar',
    careTextHint: 'Bir satır yazıp Enter’a basın',
    careLineAdd: 'Satır ekle',
    careLineEmpty: 'Henüz ek satır yok',
    careLineRemove: 'Satırı kaldır',
    careLineFull: 'Yer kalmadı — önce bir satır kaldırın.',
    careLineNumber: (index) => `${index}. satır`,
    sizes: 'Bedenler',
    addSize: 'Beden ekle',
    forgetSize: 'Bedeni unut',
    forgetStyle: 'Unut',
    learnedStyles: 'Hatırlananlar',
    color: 'Renk',
    rowTotal: 'Toplam',
    stickerQty: 'Poşet etiketi',
    colourPrice: 'Renk fiyatı (zł)',
    fabricRow: 'Kumaş etiketleri',
    addRow: 'Renk ekle',
    total: 'Genel toplam',
    whatToPrint: 'Ne yazdırılacak',
    printFabric: 'Kumaş etiketleri',
    printSticker: 'Paketleme etiketleri',
    print: 'Yazdır',
    printing: 'Yazdırılıyor…',
    samplePrint: 'Bir tane yazdır',
    ...
    noMatchingOrders: 'Eşleşen sipariş yok',
    sheetCount: (shown, total) => (shown === total ? `${total} sayfa` : `${shown}/${total} sayfa`),
    ...
    pasteOpen: 'Excel’den yapıştır',
    pasteHint: 'Excel’de bloğu seçin, Ctrl+C yapın, sonra aşağıya yapıştırın.',
    pasteRead: (colors, sizes, copies) =>
      `${colors} renk × ${sizes} beden okundu, toplam ${copies} etiket.`,
    pasteReplace: (colors, sizes) => `Ekrandaki ${colors} renk × ${sizes} bedenin yerine geçer.`,
    pasteAccept: 'Bu tabloyu kullan',
    pasteCancel: 'İptal',
    pasteProblem: {
      NOT_A_GRID: 'Bu bir tablo değil — ilk satır bedenler, sonra her renk için bir satır.',
      NO_SIZES: 'İlk satırda beden yok.',
      NO_ROWS: 'Renk satırı yok.',
    },
    duplicate: 'Kopyala',
    duplicateHint:
      'Aynı içerikle yeni bir sipariş. Siparişi kaydet’e basana kadar hiçbir şey kaydedilmez.',
    stopAfter: 'Yazdırmayı durdur',
    stopHint: 'Durdurma, gönderilmiş olan parti bittikten sonra geçerli olur.',
    noResume: 'Sıkışma veya uygulama yeniden başladıktan sonra, yeniden yazdırmadan önce basılan etiketleri sayın.',
    resumeSent: (steps, totalSteps, copies, totalCopies) =>
      `Geçen sefer ${steps}/${totalSteps} parti çıktı (${copies}/${totalCopies} etiket).`,
    resumeCount: 'Etiketleri sayın, sonra seçin:',
    resumeContinue: (batch) => `${batch}. partiden devam et`,
    resumeRestart: 'Baştan yazdır',
    resumeForget: 'İlerlemeyi unut',
    percentSum: (sum) => `Yüzdelerin toplamı: %${sum}`,
    percentFix: (name, percent) => `${name} = %${percent} yap`,
    progress: (done, total, copies, all) => `${done}/${total} parti · ${copies}/${all} etiket`,
    finished: (copies) => `${copies} etiket yazdırıldı`,
    stopped: (done, total) => `${done}/${total} partiden sonra durduruldu`,
    problem: {
      EMPTY_ORDER: 'Miktar girilmedi',
      ...
      ORDER_TOO_LARGE: 'Miktar inandırıcı olmayacak kadar büyük — sayfayı kontrol edin',
    },
    warning: {
    ...
    price: 'Brüt fiyat (zł)',
    image: 'Fotoğraf',
    imagePick: 'Fotoğraf seç',
    imageChange: 'Fotoğrafı değiştir',
    imageClear: 'Fotoğrafı kaldır',
    imageBadType: 'Yalnızca JPG, PNG veya WEBP',
    imageUploaded: (done, total) => (done === total ? ' · fotoğraf eklendi' : ` · fotoğraf ${done}/${total} satıra eklendi`),
    priceSynced: (rows) => ` · sayfadaki fiyat mevcut satırlara uygulandı: ${rows}`,
    priceSyncFailed: (reason) => `Satırlar eklendi, ancak mevcut satırlar fiyatını korudu: ${reason}`,
    nameSynced: ' · model adı değiştirildi',
    nameSyncUnsupported: ' · model adı değişmedi: sunucu modeli yeniden adlandıramıyor',
    nameSyncFailed: (reason) => `Model yeniden adlandırılamadı: ${reason}`,
    categoryNone: 'Kategori yok — yukarıdan seçin veya oluşturun, yoksa model etiket sekmesinde görünmez',
    categoryPick: '— Bir kategori seçin —',
    createCategory: (name) => `“${name}” kategorisini oluştur`,
    creatingCategory: 'Kategori oluşturuluyor…',
    categoryCreateFailed: (reason) => `Kategori oluşturulamadı: ${reason}`,
    fileProduct: 'Ürün olarak kaydet',
    filing: 'Kaydediliyor…',
    filed: (variants) => `Kaydedildi — ${variants} varyant`,
    filedWithoutTag: (variants) =>
      `${variants} varyant kaydedildi, ancak bakım içeriği kaydedilmedi — etiket sekmesi kumaş etiketi basamaz`,
    fileHint: 'Dolu her hücre bir renk ve beden varyantı olur',
    updateProduct: 'Ürünü güncelle',
    updating: 'Güncelleniyor…',
    updated: (added) => (added > 0 ? `Güncellendi — eklenen yeni renk/beden: ${added}` : 'Ürün güncellendi'),
    filedHint: (name) => `Zaten “${name}” ürünü — ürün etiketi sekmesinde açın`,
    attachProduct: (name) => `Mevcut “${name}” modeline ekle`,
    attaching: 'Ekleniyor…',
    attachHint: (name) => `Bu kod zaten “${name}” modeli — yeni model yapmak yerine eksik renk ve bedenleri ona eklemek için basın`,
    attached: (name, added) =>
      added > 0 ? `“${name}” modeline eklendi — yeni renk/beden: ${added}` : `“${name}” modeline eklendi`,
    styleUnknown: 'Bu model henüz bu makinedeki katalogda yok — ürün sekmesinde Eşitle’ye basıp tekrar deneyin',
    fileFailed: (reason) => `Kaydedilmedi: ${reason}`,
    fileProblem: {
      NO_NAME: 'Modelin adı yok',
      NO_CUSTOMER: 'Müşteri adı yok',
      NO_CATEGORY: 'Kategori seçilmedi',
      NO_CELLS: 'Henüz renk yok — her renk × beden bir varyant olur',
      ALREADY_FILED: 'Bu sayfa zaten ürün olarak kaydedilmiş',
      NO_PRICE: 'Henüz fiyat yok — tüm model için tek fiyat',
      TOO_MANY_VARIANTS: '100’den fazla varyant — siparişi bölün',
    },
  },
  zh: {
    title: '打印单',
    subtitle: '录入客户的 A4 单页，然后一次打印',
    customer: '客户',
    styleName: '款式名称',
    styleCode: '款号',
    materials: '成分',
    materialsHint: '点选材料并输入百分比',
    care: '洗涤符号',
    careGroup: { wash: '水洗', bleach: '漂白', tumble: '烘干', natural: '自然晾干', iron: '熨烫', dryclean: '干洗', wetclean: '湿洗' },
    careText: '附加行',
    careTextHint: '输入一行后按 Enter',
    careLineAdd: '添加行',
    careLineEmpty: '还没有附加行',
    careLineRemove: '删除行',
    careLineFull: '没有空位了 — 请先删除一行。',
    careLineNumber: (index) => `第 ${index} 行`,
    sizes: '尺码',
    addSize: '添加尺码',
    forgetSize: '忘记尺码',
    forgetStyle: '忘记',
    ...
    noMatchingOrders: '没有匹配的打印单',
    sheetCount: (shown, total) => (shown === total ? `${total} 张` : `${shown}/${total} 张`),
    ...
    remove: '删除',
    newOrder: '新建打印单',
    pasteOpen: '从 Excel 粘贴',
    pasteHint: '在 Excel 中选中区域，Ctrl+C，然后粘贴到下面。',
    pasteRead: (colors, sizes, copies) => `读取到 ${colors} 种颜色 × ${sizes} 个尺码，共 ${copies} 张标签。`,
    pasteReplace: (colors, sizes) => `将替换屏幕上的 ${colors} 种颜色 × ${sizes} 个尺码。`,
    pasteAccept: '使用此表格',
    pasteCancel: '取消',
    pasteProblem: {
      NOT_A_GRID: '这不是表格 — 第一行是尺码，之后每种颜色一行。',
      NO_SIZES: '第一行没有尺码。',
      NO_ROWS: '没有颜色行。',
    },
    duplicate: '复制',
    duplicateHint: '内容相同的新打印单。在按下「保存打印单」之前不会存档。',
    stopAfter: '停止打印',
    stopHint: '已发送的这一批打完后，停止才会生效。',
    noResume: '卡纸或应用重启后，请先清点已打印的标签再重印。',
    resumeSent: (steps, totalSteps, copies, totalCopies) =>
      `上次发出了 ${steps}/${totalSteps} 批（${copies}/${totalCopies} 张标签）。`,
    resumeCount: '清点标签，然后选择：',
    resumeContinue: (batch) => `从第 ${batch} 批继续`,
    resumeRestart: '从头开始打印',
    resumeForget: '忘记进度',
    percentSum: (sum) => `百分比合计为 ${sum}%`,
    percentFix: (name, percent) => `将 ${name} 设为 ${percent}%`,
    progress: (done, total, copies, all) => `${done}/${total} 批 · ${copies}/${all} 张标签`,
    finished: (copies) => `已打印 ${copies} 张标签`,
    stopped: (done, total) => `在 ${done}/${total} 批后停止`,
    problem: {
      EMPTY_ORDER: '未填写数量',
      ...
      ORDER_TOO_LARGE: '数量大得不合常理 — 请核对单页',
    },
    warning: {
    ...
    imageChange: '更换照片',
    imageClear: '移除照片',
    imageBadType: '仅支持 JPG、PNG 或 WEBP',
    imageUploaded: (done, total) => (done === total ? ' · 已附上照片' : ` · 已为 ${done}/${total} 行附上照片`),
    priceSynced: (rows) => ` · 已按单页设置现有行的价格：${rows}`,
    priceSyncFailed: (reason) => `行已添加，但现有行保留了原价格：${reason}`,
    nameSynced: ' · 款式已改名',
    nameSyncUnsupported: ' · 款式名称未变：服务器无法重命名款式',
    nameSyncFailed: (reason) => `无法重命名款式：${reason}`,
    categoryNone: '没有类别 — 请在上方选择或新建，否则款式不会出现在标签页',
    categoryPick: '— 选择类别 —',
    createCategory: (name) => `新建类别“${name}”`,
    creatingCategory: '正在新建类别…',
    categoryCreateFailed: (reason) => `无法新建类别：${reason}`,
    fileProduct: '保存为商品',
    filing: '正在保存…',
    filed: (variants) => `已保存 — ${variants} 个变体`,
    filedWithoutTag: (variants) => `已保存 ${variants} 个变体，但洗涤内容未保存 — 标签页无法打印布标`,
    fileHint: '每个填写的单元格会成为一个颜色和尺码的变体',
    updateProduct: '更新商品',
    updating: '正在更新…',
    updated: (added) => (added > 0 ? `已更新 — 新增颜色/尺码：${added}` : '商品已更新'),
    filedHint: (name) => `已经是商品“${name}” — 请在商品标签页打开`,
    attachProduct: (name) => `并入已有的“${name}”`,
    attaching: '正在并入…',
    attachHint: (name) => `此款号已是款式“${name}” — 按此把缺少的颜色和尺码加进去，而不是新建款式`,
    attached: (name, added) => (added > 0 ? `已并入“${name}” — 新增颜色/尺码：${added}` : `已并入“${name}”`),
    styleUnknown: '本机的商品目录里还没有这个款式 — 请在商品页按同步后再试',
    fileFailed: (reason) => `未保存：${reason}`,
    fileProblem: {
      NO_NAME: '款式没有名称',
      NO_CUSTOMER: '没有客户名称',
      NO_CATEGORY: '未选择类别',
      NO_CELLS: '还没有颜色 — 每个颜色 × 尺码会成为一个变体',
      ALREADY_FILED: '此单页已保存为商品',
      NO_PRICE: '还没有价格 — 整个款式一个价',
      TOO_MANY_VARIANTS: '超过 100 个变体 — 请拆分订单',
    },
  },
  uk: {
    title: 'Замовлення на друк',
    subtitle: 'Введіть аркуш A4 клієнта, потім надрукуйте за один раз',
    customer: 'Клієнт',
    styleName: 'Назва моделі',
    styleCode: 'Код моделі',
    materials: 'Склад',
    materialsHint: 'Торкніться матеріалу і введіть відсоток',
    care: 'Символи догляду',
    careGroup: { wash: 'Прання', bleach: 'Відбілювання', tumble: 'Сушильна машина', natural: 'Природне сушіння', iron: 'Прасування', dryclean: 'Хімчистка', wetclean: 'Аквачистка' },
    careText: 'Додаткові рядки',
    careTextHint: 'Введіть рядок і натисніть Enter',
    careLineAdd: 'Додати рядок',
    careLineEmpty: 'Додаткових рядків ще немає',
    careLineRemove: 'Прибрати рядок',
    careLineFull: 'Місця немає — спершу приберіть рядок.',
    careLineNumber: (index) => `Рядок ${index}`,
    sizes: 'Розміри',
    addSize: 'Додати розмір',
    forgetSize: 'Забути розмір',
    forgetStyle: 'Забути',
    learnedStyles: 'Запам’ятані',
    color: 'Колір',
    rowTotal: 'Разом',
    stickerQty: 'Наліпки на пакет',
    colourPrice: 'Ціна кольору (zł)',
    fabricRow: 'Текстильні бирки',
    addRow: 'Додати колір',
    total: 'Загалом',
    whatToPrint: 'Що друкувати',
    printFabric: 'Текстильні бирки',
    printSticker: 'Пакувальні наліпки',
    print: 'Друк',
    printing: 'Друкуємо…',
    samplePrint: 'Надрукувати одну',
    ...
    noMatchingOrders: 'Немає замовлень за запитом',
    sheetCount: (shown, total) =>
      shown === total
        ? `${total} ${eastSlavicForm(total, 'аркуш', 'аркуші', 'аркушів')}`
        : `${shown}/${total} ${eastSlavicForm(total, 'аркуш', 'аркуші', 'аркушів')}`,
    pageOf: (page, pages) => `Сторінка ${page}/${pages}`,
    prevPage: '← Назад',
    nextPage: 'Далі →',
    open: 'Відкрити',
    remove: 'Видалити',
    newOrder: 'Нове замовлення',
    pasteOpen: 'Вставити з Excel',
    pasteHint: 'Виділіть блок в Excel, Ctrl+C, потім вставте нижче.',
    pasteRead: (colors, sizes, copies) =>
      `Зчитано ${colors} ${eastSlavicForm(colors, 'колір', 'кольори', 'кольорів')} × ${sizes} ${eastSlavicForm(sizes, 'розмір', 'розміри', 'розмірів')}, усього ${copies} ${eastSlavicForm(copies, 'етикетка', 'етикетки', 'етикеток')}.`,
    pasteReplace: (colors, sizes) =>
      `Замінить ${colors} ${eastSlavicForm(colors, 'колір', 'кольори', 'кольорів')} × ${sizes} ${eastSlavicForm(sizes, 'розмір', 'розміри', 'розмірів')} на екрані.`,
    pasteAccept: 'Використати цю таблицю',
    pasteCancel: 'Скасувати',
    pasteProblem: {
      NOT_A_GRID: 'Це не таблиця — у першому рядку розміри, далі по рядку на колір.',
      NO_SIZES: 'У першому рядку немає розмірів.',
      NO_ROWS: 'Немає рядків із кольорами.',
    },
    duplicate: 'Дублювати',
    duplicateHint: 'Нове замовлення з тим самим вмістом. Нічого не зберігається, доки не натиснете «Зберегти замовлення».',
    stopAfter: 'Зупинити друк',
    stopHint: 'Зупинка спрацює після того, як допрацює вже надіслана партія.',
    noResume: 'Після зам’яття паперу чи перезапуску застосунку перерахуйте надруковані етикетки, перш ніж друкувати знову.',
    resumeSent: (steps, totalSteps, copies, totalCopies) =>
      `Минулого разу вийшло ${steps}/${totalSteps} партій (${copies}/${totalCopies} етикеток).`,
    resumeCount: 'Перерахуйте етикетки, потім оберіть:',
    resumeContinue: (batch) => `Продовжити з партії ${batch}`,
    resumeRestart: 'Друкувати спочатку',
    resumeForget: 'Забути поступ',
    percentSum: (sum) => `Сума відсотків: ${sum}%`,
    percentFix: (name, percent) => `Встановити ${name} = ${percent}%`,
    progress: (done, total, copies, all) => `${done}/${total} партій · ${copies}/${all} етикеток`,
    finished: (copies) => `Надруковано ${copies} ${eastSlavicForm(copies, 'етикетку', 'етикетки', 'етикеток')}`,
    stopped: (done, total) => `Зупинено після ${done}/${total} партій`,
    problem: {
      EMPTY_ORDER: 'Кількість не введено',
      ...
      ORDER_TOO_LARGE: 'Кількість неправдоподібно велика — перевірте аркуш',
    },
    warning: {
    ...
    category: 'Категорія',
    price: 'Ціна брутто (zł)',
    image: 'Фото',
    imagePick: 'Обрати фото',
    imageChange: 'Змінити фото',
    imageClear: 'Прибрати фото',
    imageBadType: 'Лише JPG, PNG або WEBP',
    imageUploaded: (done, total) => (done === total ? ' · фото додано' : ` · фото додано до ${done}/${total} рядків`),
    priceSynced: (rows) => ` · ціну з аркуша застосовано до наявних рядків: ${rows}`,
    priceSyncFailed: (reason) => `Рядки додано, але наявні рядки зберегли свою ціну: ${reason}`,
    nameSynced: ' · модель перейменовано',
    nameSyncUnsupported: ' · назва моделі не змінилася: сервер не вміє перейменовувати модель',
    nameSyncFailed: (reason) => `Не вдалося перейменувати модель: ${reason}`,
    categoryNone: 'Немає категорії — оберіть її вище або створіть, інакше модель не з’явиться на вкладці «Етикетка»',
    categoryPick: '— Оберіть категорію —',
    createCategory: (name) => `Створити категорію «${name}»`,
    creatingCategory: 'Створюємо категорію…',
    categoryCreateFailed: (reason) => `Не вдалося створити категорію: ${reason}`,
    fileProduct: 'Зберегти як товар',
    filing: 'Зберігаємо…',
    filed: (variants) => `Збережено — ${variants} ${eastSlavicForm(variants, 'варіант', 'варіанти', 'варіантів')}`,
    filedWithoutTag: (variants) =>
      `Збережено ${variants} ${eastSlavicForm(variants, 'варіант', 'варіанти', 'варіантів')}, але вміст догляду не зберігся — вкладка «Етикетка» не надрукує текстильну бирку`,
    fileHint: 'Кожна заповнена клітинка стає варіантом кольору і розміру',
    updateProduct: 'Оновити товар',
    updating: 'Оновлюємо…',
    updated: (added) => (added > 0 ? `Оновлено — додано нових кольорів/розмірів: ${added}` : 'Товар оновлено'),
    filedHint: (name) => `Це вже товар «${name}» — відкрийте його на вкладці етикеток товару`,
    attachProduct: (name) => `Додати до наявної «${name}»`,
    attaching: 'Додаємо…',
    attachHint: (name) => `Цей код уже належить моделі «${name}» — натисніть, щоб додати до неї відсутні кольори й розміри замість створення нової моделі`,
    attached: (name, added) =>
      added > 0 ? `Додано до «${name}» — нових кольорів/розмірів: ${added}` : `Додано до «${name}»`,
    styleUnknown: 'Цієї моделі ще немає в каталозі на цій машині — натисніть «Синхронізувати» на вкладці товарів і спробуйте ще раз',
    fileFailed: (reason) => `Не збережено: ${reason}`,
    fileProblem: {
      NO_NAME: 'Модель без назви',
      NO_CUSTOMER: 'Немає імені клієнта',
      NO_CATEGORY: 'Категорію не обрано',
      NO_CELLS: 'Ще немає кольору — кожен колір × розмір стає варіантом',
      ALREADY_FILED: 'Цей аркуш уже збережено як товар',
      NO_PRICE: 'Ще немає ціни — одна ціна на всю модель',
      TOO_MANY_VARIANTS: 'Понад 100 варіантів — поділіть замовлення',
    },
  },
  ru: {
    title: 'Заказ на печать',
    subtitle: 'Введите лист A4 клиента, затем напечатайте за один раз',
    customer: 'Клиент',
    styleName: 'Название модели',
    styleCode: 'Код модели',
    materials: 'Состав',
    materialsHint: 'Нажмите на материал и введите процент',
    care: 'Символы ухода',
    careGroup: { wash: 'Стирка', bleach: 'Отбеливание', tumble: 'Сушильная машина', natural: 'Естественная сушка', iron: 'Глажка', dryclean: 'Химчистка', wetclean: 'Аквачистка' },
    careText: 'Дополнительные строки',
    careTextHint: 'Введите строку и нажмите Enter',
    careLineAdd: 'Добавить строку',
    careLineEmpty: 'Дополнительных строк пока нет',
    careLineRemove: 'Убрать строку',
    careLineFull: 'Места нет — сначала уберите строку.',
    careLineNumber: (index) => `Строка ${index}`,
    sizes: 'Размеры',
    addSize: 'Добавить размер',
    forgetSize: 'Забыть размер',
    forgetStyle: 'Забыть',
    learnedStyles: 'Запомненные',
    color: 'Цвет',
    rowTotal: 'Итого',
    stickerQty: 'Наклейки на пакет',
    colourPrice: 'Цена цвета (zł)',
    fabricRow: 'Текстильные бирки',
    addRow: 'Добавить цвет',
    total: 'Всего',
    whatToPrint: 'Что печатать',
    printFabric: 'Текстильные бирки',
    printSticker: 'Упаковочные наклейки',
    print: 'Печать',
    printing: 'Печатаем…',
    samplePrint: 'Напечатать одну',
    ...
    sheetCount: (shown, total) =>
      shown === total
        ? `${total} ${eastSlavicForm(total, 'лист', 'листа', 'листов')}`
        : `${shown}/${total} ${eastSlavicForm(total, 'лист', 'листа', 'листов')}`,
    pageOf: (page, pages) => `Страница ${page}/${pages}`,
    prevPage: '← Назад',
    nextPage: 'Далее →',
    open: 'Открыть',
    remove: 'Удалить',
    newOrder: 'Новый заказ',
    pasteOpen: 'Вставить из Excel',
    pasteHint: 'Выделите блок в Excel, Ctrl+C, затем вставьте ниже.',
    pasteRead: (colors, sizes, copies) =>
      `Считано ${colors} ${eastSlavicForm(colors, 'цвет', 'цвета', 'цветов')} × ${sizes} ${eastSlavicForm(sizes, 'размер', 'размера', 'размеров')}, всего ${copies} ${eastSlavicForm(copies, 'этикетка', 'этикетки', 'этикеток')}.`,
    pasteReplace: (colors, sizes) =>
      `Заменит ${colors} ${eastSlavicForm(colors, 'цвет', 'цвета', 'цветов')} × ${sizes} ${eastSlavicForm(sizes, 'размер', 'размера', 'размеров')} на экране.`,
    pasteAccept: 'Использовать эту таблицу',
    pasteCancel: 'Отмена',
    pasteProblem: {
      NOT_A_GRID: 'Это не таблица — в первой строке размеры, дальше по строке на цвет.',
      NO_SIZES: 'В первой строке нет размеров.',
      NO_ROWS: 'Нет строк с цветами.',
    },
    duplicate: 'Дублировать',
    duplicateHint: 'Новый заказ с тем же содержимым. Ничего не сохраняется, пока не нажмёте «Сохранить заказ».',
    stopAfter: 'Остановить печать',
    stopHint: 'Остановка сработает после того, как допечатается уже отправленная партия.',
    noResume: 'После замятия бумаги или перезапуска приложения пересчитайте напечатанные этикетки, прежде чем печатать снова.',
    resumeSent: (steps, totalSteps, copies, totalCopies) =>
      `В прошлый раз вышло ${steps}/${totalSteps} партий (${copies}/${totalCopies} этикеток).`,
    resumeCount: 'Пересчитайте этикетки, затем выберите:',
    resumeContinue: (batch) => `Продолжить с партии ${batch}`,
    resumeRestart: 'Печатать сначала',
    resumeForget: 'Забыть прогресс',
    percentSum: (sum) => `Сумма процентов: ${sum}%`,
    percentFix: (name, percent) => `Установить ${name} = ${percent}%`,
    progress: (done, total, copies, all) => `${done}/${total} партий · ${copies}/${all} этикеток`,
    finished: (copies) => `Напечатано ${copies} ${eastSlavicForm(copies, 'этикетка', 'этикетки', 'этикеток')}`,
    stopped: (done, total) => `Остановлено после ${done}/${total} партий`,
    problem: {
      EMPTY_ORDER: 'Количество не введено',
      ...
      ORDER_TOO_LARGE: 'Количество неправдоподобно велико — проверьте лист',
    },
    warning: {
    ...
    category: 'Категория',
    price: 'Цена брутто (zł)',
    image: 'Фото',
    imagePick: 'Выбрать фото',
    imageChange: 'Заменить фото',
    imageClear: 'Убрать фото',
    imageBadType: 'Только JPG, PNG или WEBP',
    imageUploaded: (done, total) => (done === total ? ' · фото прикреплено' : ` · фото прикреплено к ${done}/${total} строкам`),
    priceSynced: (rows) => ` · цена с листа применена к существующим строкам: ${rows}`,
    priceSyncFailed: (reason) => `Строки добавлены, но существующие строки сохранили свою цену: ${reason}`,
    nameSynced: ' · модель переименована',
    nameSyncUnsupported: ' · название модели не изменилось: сервер не умеет переименовывать модель',
    nameSyncFailed: (reason) => `Не удалось переименовать модель: ${reason}`,
    categoryNone: 'Нет категории — выберите её выше или создайте, иначе модель не появится на вкладке «Этикетка»',
    categoryPick: '— Выберите категорию —',
    createCategory: (name) => `Создать категорию «${name}»`,
    creatingCategory: 'Создаём категорию…',
    categoryCreateFailed: (reason) => `Не удалось создать категорию: ${reason}`,
    fileProduct: 'Сохранить как товар',
    filing: 'Сохраняем…',
    filed: (variants) => `Сохранено — ${variants} ${eastSlavicForm(variants, 'вариант', 'варианта', 'вариантов')}`,
    filedWithoutTag: (variants) =>
      `Сохранено ${variants} ${eastSlavicForm(variants, 'вариант', 'варианта', 'вариантов')}, но содержимое ухода не сохранилось — вкладка «Этикетка» не напечатает текстильную бирку`,
    fileHint: 'Каждая заполненная ячейка становится вариантом цвета и размера',
    updateProduct: 'Обновить товар',
    updating: 'Обновляем…',
    updated: (added) => (added > 0 ? `Обновлено — добавлено новых цветов/размеров: ${added}` : 'Товар обновлён'),
    filedHint: (name) => `Это уже товар «${name}» — откройте его на вкладке этикеток товара`,
    attachProduct: (name) => `Добавить к существующей «${name}»`,
    attaching: 'Добавляем…',
    attachHint: (name) => `Этот код уже принадлежит модели «${name}» — нажмите, чтобы добавить к ней недостающие цвета и размеры вместо создания новой модели`,
    attached: (name, added) =>
      added > 0 ? `Добавлено к «${name}» — новых цветов/размеров: ${added}` : `Добавлено к «${name}»`,
    styleUnknown: 'Этой модели ещё нет в каталоге на этой машине — нажмите «Синхронизировать» на вкладке товаров и попробуйте снова',
    fileFailed: (reason) => `Не сохранено: ${reason}`,
    fileProblem: {
      NO_NAME: 'У модели нет названия',
      NO_CUSTOMER: 'Нет имени клиента',
      NO_CATEGORY: 'Категория не выбрана',
      NO_CELLS: 'Цвета ещё нет — каждый цвет × размер становится вариантом',
      ALREADY_FILED: 'Этот лист уже сохранён как товар',
      NO_PRICE: 'Цены ещё нет — одна цена на всю модель',
      TOO_MANY_VARIANTS: 'Более 100 вариантов — разделите заказ',
    },
  },
};
