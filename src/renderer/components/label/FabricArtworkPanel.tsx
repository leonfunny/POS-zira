...
const COPY: Partial<Record<Language, Copy>> & { en: Copy; vi: Copy; pl: Copy } = {
  ...
  tr: {
    title: 'Müşteri dosyalarından kumaş etiketleri',
    subtitle: 'Kaynağı saklayın, bir üretim PNG’si ekleyin, adet girin, sonra yazdırın',
    fixedMedia: 'Sabit 20 mm genişlik · 203 dpi · otomatik ölçekleme yok',
    fixedMediaHint: 'Bir görsel ancak medya doğrulamasından sonra Hazır olur. Uygulama görseli sığdırmak için asla germez veya küçültmez.',
    productionPngContract: 'Gerekli üretim PNG’si: tam 160 px genişlik, 80–480 px yükseklik, sol ve sağda 9 px beyaz kenar boşluğu; yazdırılabilir orta alan 142 px. 203 dpi’de 20 mm, ölçekleme yok.',
    unavailableTitle: 'Kumaş görsel kütüphanesi kullanılamıyor',
    unavailableBody: 'Kumaş baskısı güvenli şekilde devre dışı. Ürün kodu / EAN etiketleri yukarıda kullanılabilir durumda.',
    importTitle: 'Bir baskı siparişi için dosya al',
    importHint: 'Önce bilgileri girin, sonra bir .btw veya .png dosyası seçin. Dosya yolları asla elle yazılmaz.',
    customer: 'Müşteri',
    customerPlaceholder: 'Örneğin: MOON',
    orderCode: 'Sipariş / model kodu',
    orderCodePlaceholder: 'İsteğe bağlı',
    variant: 'Etikete basılan beden / varyant',
    variantPlaceholder: 'M, S/M, 44/46, L/XL…',
    revision: 'Revizyon',
    chooseFile: '.btw veya .png seç',
    importing: 'Alınıyor…',
    requiredFields: 'Müşteri, beden/varyant ve geçerli bir revizyon zorunludur.',
    libraryTitle: 'Alınan dosya kütüphanesi',
    search: 'Müşteri, sipariş, beden veya dosya adı ara',
    all: 'Tümü',
    ready: 'Hazır',
    needsPng: 'Baskı PNG’si gerekli',
    loading: 'Kütüphane yükleniyor…',
    loadError: 'Kumaş görsel kütüphanesi yüklenemedi.',
    retry: 'Tekrar dene',
    empty: 'Henüz dosya yok. İlk dosyayı yukarıdan alın.',
    emptySearch: 'Geçerli filtreye uyan dosya yok.',
    source: 'Kaynak',
    production: 'Üretim dosyası',
    noProduction: 'PNG eklenmedi',
    geometry: 'Üretim geometrisi',
    copies: 'Adet',
    selectForPrint: 'Yazdırmak için seç',
    preview: 'Önizleme',
    attachPng: '20 mm PNG ekle',
    attaching: 'Ekleniyor…',
    retire: 'Kullanımdan kaldır',
    retireTitle: 'Bu dosya kullanımdan kaldırılsın mı?',
    retireBody: 'Dosya yeni baskı seçimlerinde görünmez. Geçmişi korunur.',
    retireConfirm: 'Kaldır',
    cancel: 'İptal',
    needsConversionHint: '.btw kaynağı saklandı ancak henüz basılamaz. BarTender’dan 20 mm / 203 dpi PNG dışa aktarıp ekleyin.',
    readyHint: 'Üretim PNG’si medya doğrulamasını geçti ve yazdırmak için seçilebilir.',
    previewTitle: 'Üretim görseli önizlemesi',
    noPreviewSelection: 'Müşteri, beden ve revizyonu doğrulamak için bir satırda Önizleme’yi seçin.',
    previewLoading: 'Önizleme yükleniyor…',
    previewUnavailable: 'Bu kaydın henüz üretim PNG’si yok.',
    reviewTitle: 'Baskı işini gözden geçir',
    selectedCount: (count) => `${count} satır seçildi`,
    nothingSelected: 'En az bir Hazır satır seçin ve adedini girin.',
    total: 'Toplam etiket',
    quantityInvalid: 'Her satırın adedi 1 ile 999 arasında tam sayı olmalıdır.',
    printSelected: 'Seçili satırları yazdır',
    printing: 'Sırayla yazdırılıyor…',
    stopAfterCurrent: 'Bu parti bitince dur',
    continueNextChunk: 'Sonraki partiye devam et',
    stopNow: 'Burada dur',
    awaitingNextChunk: (done, chunks) => `${done}/${chunks} parti gönderildi. Son etiketleri kontrol edip Devam veya Dur seçin.`,
    progress: (done, chunks, copies, total) => `${done}/${chunks} parti · ${copies}/${total} etiket gönderildi`,
    highTitle: 'Büyük baskı işini onayla',
    highBody: (count) => `${count} kumaş etiketi göndermek üzeresiniz. Dosyaları, bedenleri ve miktarları yeniden kontrol edin.`,
    confirmPrint: 'Baskıyı onayla',
    printed: (count) => `${count} etiketin tamamı yazıcıya gönderildi.`,
    stopped: (done, total) => `${done}/${total} partiden sonra durduruldu.`,
    printFailed: 'Baskı başarısız olduğu için durduruldu.',
    noDurableResume: 'Henüz kalıcı bir devam noktası yok: uygulama kapandıktan veya yazıcı sıkıştıktan sonra, yeniden yazdırmadan önce fiziksel etiketleri sayın.',
    imported: 'Kaynak dosya kütüphaneye kaydedildi.',
    productionAttached: 'Üretim PNG’si eklendi.',
    retired: 'Dosya kullanımdan kaldırıldı.',
    unknownCustomer: 'Bilinmeyen müşteri',
  },
  zh: {
    title: '来自客户文件的布标',
    subtitle: '保存源文件，附上生产用 PNG，填写份数，然后打印',
    fixedMedia: '固定 20 mm 宽 · 203 dpi · 不自动缩放',
    fixedMediaHint: '图像只有通过介质校验后才会变为「就绪」。应用绝不会为了适配而拉伸或缩小图稿。',
    productionPngContract: '生产用 PNG 要求：宽度正好 160 px，高度 80–480 px，左右各留 9 px 白边；可打印中间区域为 142 px。203 dpi 下 20 mm，不缩放。',
    unavailableTitle: '布标图稿库不可用',
    unavailableBody: '布标打印已安全停用。上方的商品编码 / EAN 标签仍可使用。',
    importTitle: '为打印单接收文件',
    importHint: '先填写信息，再选择 .btw 或 .png 文件。文件路径绝不手动输入。',
    customer: '客户',
    customerPlaceholder: '例如：MOON',
    orderCode: '订单 / 款号',
    orderCodePlaceholder: '可选',
    variant: '标签上打印的尺码 / 变体',
    variantPlaceholder: 'M、S/M、44/46、L/XL…',
    revision: '版本',
    chooseFile: '选择 .btw 或 .png',
    importing: '正在导入…',
    requiredFields: '客户、尺码/变体和有效版本为必填项。',
    libraryTitle: '已接收文件库',
    search: '搜索客户、订单、尺码或文件名',
    all: '全部',
    ready: '就绪',
    needsPng: '缺少打印 PNG',
    loading: '正在加载文件库…',
    loadError: '无法加载布标图稿库。',
    retry: '重试',
    empty: '还没有文件。请先在上方接收第一个文件。',
    emptySearch: '没有符合当前筛选的文件。',
    source: '源文件',
    production: '生产文件',
    noProduction: '未附 PNG',
    geometry: '生产尺寸',
    copies: '份数',
    selectForPrint: '选择以打印',
    preview: '预览',
    attachPng: '附上 20 mm PNG',
    attaching: '正在附加…',
    retire: '停用',
    retireTitle: '停用此文件？',
    retireBody: '该文件将不再出现在新的打印选择中，历史记录会保留。',
    retireConfirm: '停用',
    cancel: '取消',
    needsConversionHint: '.btw 源文件已保存，但还不能打印。请从 BarTender 导出 20 mm / 203 dpi 的 PNG 并附上。',
    readyHint: '生产用 PNG 已通过介质校验，可选择打印。',
    previewTitle: '生产图稿预览',
    noPreviewSelection: '在某一行选择「预览」，核对客户、尺码和版本。',
    previewLoading: '正在加载预览…',
    previewUnavailable: '此记录还没有生产用 PNG。',
    reviewTitle: '核对打印任务',
    selectedCount: (count) => `已选 ${count} 行`,
    nothingSelected: '请至少选择一行「就绪」并填写份数。',
    total: '标签总数',
    quantityInvalid: '每行份数必须是 1 到 999 之间的整数。',
    printSelected: '打印所选行',
    printing: '正在依次打印…',
    stopAfterCurrent: '当前批次后停止',
    continueNextChunk: '继续下一批',
    stopNow: '就此停止',
    awaitingNextChunk: (done, chunks) => `已提交 ${done}/${chunks} 批。检查最新标签后选择「继续」或「停止」。`,
    progress: (done, chunks, copies, total) => `${done}/${chunks} 批 · 已提交 ${copies}/${total} 张标签`,
    highTitle: '确认大批量打印',
    highBody: (count) => `即将提交 ${count} 张布标。请再次核对文件、尺码和数量。`,
    confirmPrint: '确认打印',
    printed: (count) => `全部 ${count} 张标签已提交到打印机。`,
    stopped: (done, total) => `在 ${done}/${total} 批后停止。`,
    printFailed: '因打印失败而停止。',
    noDurableResume: '目前还没有可靠的断点续打：应用关闭或卡纸后，请先清点实物标签再重新打印。',
    imported: '源文件已保存到文件库。',
    productionAttached: '生产用 PNG 已附上。',
    retired: '文件已停用。',
    unknownCustomer: '未知客户',
  },
  uk: {
    title: 'Текстильні бирки з файлів клієнта',
    subtitle: 'Збережіть джерело, додайте виробничий PNG, вкажіть кількість, потім друкуйте',
    fixedMedia: 'Фіксована ширина 20 мм · 203 dpi · без автоматичного масштабування',
    fixedMediaHint: 'Зображення стає «Готово» лише після перевірки носія. Застосунок ніколи не розтягує і не стискає макет, щоб він вліз.',
    productionPngContract: 'Потрібний виробничий PNG: рівно 160 px завширшки, 80–480 px заввишки, з білими полями по 9 px ліворуч і праворуч; друкована середина — 142 px. 20 мм при 203 dpi, без масштабування.',
    unavailableTitle: 'Бібліотека макетів недоступна',
    unavailableBody: 'Друк текстильних бирок безпечно вимкнено. Етикетки з кодом товару / EAN вище лишаються доступними.',
    importTitle: 'Отримати файл для замовлення на друк',
    importHint: 'Спершу введіть дані, потім оберіть файл .btw або .png. Шляхи до файлів ніколи не вводяться вручну.',
    customer: 'Клієнт',
    customerPlaceholder: 'Наприклад: MOON',
    orderCode: 'Код замовлення / моделі',
    orderCodePlaceholder: 'Необов’язково',
    variant: 'Розмір / варіант, надрукований на бирці',
    variantPlaceholder: 'M, S/M, 44/46, L/XL…',
    revision: 'Ревізія',
    chooseFile: 'Обрати .btw або .png',
    importing: 'Імпортуємо…',
    requiredFields: 'Клієнт, розмір/варіант і коректна ревізія обов’язкові.',
    libraryTitle: 'Бібліотека отриманих файлів',
    search: 'Пошук за клієнтом, замовленням, розміром або назвою файлу',
    all: 'Усі',
    ready: 'Готово',
    needsPng: 'Потрібен PNG для друку',
    loading: 'Завантаження бібліотеки…',
    loadError: 'Не вдалося завантажити бібліотеку макетів.',
    retry: 'Спробувати ще раз',
    empty: 'Файлів ще немає. Отримайте перший файл вище.',
    emptySearch: 'Немає файлів за поточним фільтром.',
    source: 'Джерело',
    production: 'Виробничий файл',
    noProduction: 'PNG не додано',
    geometry: 'Виробнича геометрія',
    copies: 'Кількість',
    selectForPrint: 'Обрати для друку',
    preview: 'Перегляд',
    attachPng: 'Додати PNG 20 мм',
    attaching: 'Додаємо…',
    retire: 'Вивести з обігу',
    retireTitle: 'Вивести цей файл з обігу?',
    retireBody: 'Файл зникне з нових виборів для друку. Його історія збережеться.',
    retireConfirm: 'Вивести',
    cancel: 'Скасувати',
    needsConversionHint: 'Джерело .btw збережено, але друкувати ще не можна. Експортуйте PNG 20 мм / 203 dpi з BarTender і додайте його.',
    readyHint: 'Виробничий PNG пройшов перевірку носія і його можна обрати для друку.',
    previewTitle: 'Перегляд виробничого макета',
    noPreviewSelection: 'Натисніть «Перегляд» у рядку, щоб перевірити клієнта, розмір і ревізію.',
    previewLoading: 'Завантаження перегляду…',
    previewUnavailable: 'У цього запису ще немає виробничого PNG.',
    reviewTitle: 'Перевірити партію друку',
    selectedCount: (count) => `Обрано рядків: ${count}`,
    nothingSelected: 'Оберіть щонайменше один готовий рядок і вкажіть кількість.',
    total: 'Усього етикеток',
    quantityInvalid: 'Кожен рядок має мати цілу кількість від 1 до 999.',
    printSelected: 'Друкувати обрані рядки',
    printing: 'Друкуємо послідовно…',
    stopAfterCurrent: 'Зупинити після поточної частини',
    continueNextChunk: 'Продовжити наступну частину',
    stopNow: 'Зупинити тут',
    awaitingNextChunk: (done, chunks) => `Надіслано ${done}/${chunks} частин. Перевірте останні етикетки, потім оберіть «Продовжити» або «Зупинити».`,
    progress: (done, chunks, copies, total) => `${done}/${chunks} частин · надіслано ${copies}/${total} етикеток`,
    highTitle: 'Підтвердьте великий тираж',
    highBody: (count) => `Ви збираєтеся надіслати ${count} текстильних бирок. Перевірте файли, розміри й кількості ще раз.`,
    confirmPrint: 'Підтвердити друк',
    printed: (count) => `Усі ${count} етикеток надіслано на принтер.`,
    stopped: (done, total) => `Зупинено після ${done}/${total} частин.`,
    printFailed: 'Зупинено, бо друк не вдався.',
    noDurableResume: 'Надійного продовження ще немає: після закриття застосунку чи зам’яття перерахуйте фізичні етикетки, перш ніж друкувати знову.',
    imported: 'Вихідний файл збережено в бібліотеці.',
    productionAttached: 'Виробничий PNG додано.',
    retired: 'Файл виведено з обігу.',
    unknownCustomer: 'Невідомий клієнт',
  },
  ru: {
    title: 'Текстильные бирки из файлов клиента',
    subtitle: 'Сохраните исходник, приложите производственный PNG, укажите количество, затем печатайте',
    fixedMedia: 'Фиксированная ширина 20 мм · 203 dpi · без автоматического масштабирования',
    fixedMediaHint: 'Изображение становится «Готово» только после проверки носителя. Приложение никогда не растягивает и не сжимает макет, чтобы он поместился.',
    productionPngContract: 'Требуемый производственный PNG: ровно 160 px в ширину, 80–480 px в высоту, с белыми полями по 9 px слева и справа; печатаемая середина — 142 px. 20 мм при 203 dpi, без масштабирования.',
    unavailableTitle: 'Библиотека макетов недоступна',
    unavailableBody: 'Печать текстильных бирок безопасно отключена. Этикетки с кодом товара / EAN выше остаются доступными.',
    importTitle: 'Получить файл для заказа на печать',
    importHint: 'Сначала введите данные, затем выберите файл .btw или .png. Пути к файлам никогда не вводятся вручную.',
    customer: 'Клиент',
    customerPlaceholder: 'Например: MOON',
    orderCode: 'Код заказа / модели',
    orderCodePlaceholder: 'Необязательно',
    variant: 'Размер / вариант, печатаемый на бирке',
    variantPlaceholder: 'M, S/M, 44/46, L/XL…',
    revision: 'Ревизия',
    chooseFile: 'Выбрать .btw или .png',
    importing: 'Импортируем…',
    requiredFields: 'Клиент, размер/вариант и корректная ревизия обязательны.',
    libraryTitle: 'Библиотека полученных файлов',
    search: 'Поиск по клиенту, заказу, размеру или имени файла',
    all: 'Все',
    ready: 'Готово',
    needsPng: 'Нужен PNG для печати',
    loading: 'Загрузка библиотеки…',
    loadError: 'Не удалось загрузить библиотеку макетов.',
    retry: 'Повторить',
    empty: 'Файлов пока нет. Получите первый файл выше.',
    emptySearch: 'Нет файлов по текущему фильтру.',
    source: 'Исходник',
    production: 'Производственный файл',
    noProduction: 'PNG не приложен',
    geometry: 'Производственная геометрия',
    copies: 'Количество',
    selectForPrint: 'Выбрать для печати',
    preview: 'Просмотр',
    attachPng: 'Приложить PNG 20 мм',
    attaching: 'Прикладываем…',
    retire: 'Вывести из обращения',
    retireTitle: 'Вывести этот файл из обращения?',
    retireBody: 'Файл исчезнет из новых выборов для печати. Его история сохранится.',
    retireConfirm: 'Вывести',
    cancel: 'Отмена',
    needsConversionHint: 'Исходник .btw сохранён, но печатать ещё нельзя. Экспортируйте PNG 20 мм / 203 dpi из BarTender и приложите его.',
    readyHint: 'Производственный PNG прошёл проверку носителя и его можно выбрать для печати.',
    previewTitle: 'Просмотр производственного макета',
    noPreviewSelection: 'Нажмите «Просмотр» в строке, чтобы проверить клиента, размер и ревизию.',
    previewLoading: 'Загрузка просмотра…',
    previewUnavailable: 'У этой записи ещё нет производственного PNG.',
    reviewTitle: 'Проверить партию печати',
    selectedCount: (count) => `Выбрано строк: ${count}`,
    nothingSelected: 'Выберите хотя бы одну готовую строку и укажите количество.',
    total: 'Всего этикеток',
    quantityInvalid: 'В каждой строке количество должно быть целым числом от 1 до 999.',
    printSelected: 'Печатать выбранные строки',
    printing: 'Печатаем последовательно…',
    stopAfterCurrent: 'Остановить после текущей части',
    continueNextChunk: 'Продолжить следующую часть',
    stopNow: 'Остановить здесь',
    awaitingNextChunk: (done, chunks) => `Отправлено ${done}/${chunks} частей. Проверьте последние этикетки, затем выберите «Продолжить» или «Остановить».`,
    progress: (done, chunks, copies, total) => `${done}/${chunks} частей · отправлено ${copies}/${total} этикеток`,
    highTitle: 'Подтвердите большой тираж',
    highBody: (count) => `Вы собираетесь отправить ${count} текстильных бирок. Перепроверьте файлы, размеры и количества.`,
    confirmPrint: 'Подтвердить печать',
    printed: (count) => `Все ${count} этикеток отправлены на принтер.`,
    stopped: (done, total) => `Остановлено после ${done}/${total} частей.`,
    printFailed: 'Остановлено, потому что печать не удалась.',
    noDurableResume: 'Надёжного продолжения пока нет: после закрытия приложения или замятия пересчитайте физические этикетки, прежде чем печатать снова.',
    imported: 'Исходный файл сохранён в библиотеке.',
    productionAttached: 'Производственный PNG приложен.',
    retired: 'Файл выведен из обращения.',
