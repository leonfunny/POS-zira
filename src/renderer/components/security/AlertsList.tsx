...
export default function AlertsList({ limit = 50, cameraId }: AlertsListProps) {
  ...
  const loadAlerts = useCallback(async () => {
    try {
      ...
      console.error('[AlertsList] Failed to load:', err);
    } finally {
      setLoading(false);
    }
  }, [limit, cameraId]);
  ...
  if (loading) {
    return <div className="text-sm text-slate-500 p-4">Loading alerts...</div>;
  }
      ...
      {alerts.length === 0 ? (
        ...
        <div className="space-y-1 max-h-[400px] overflow-y-auto">
          {alerts.map((alert) => (
            ...
            >
              <span className="text-sm mt-0.5">{SEVERITY_ICON[alert.severity] || '🟢'}</span>
