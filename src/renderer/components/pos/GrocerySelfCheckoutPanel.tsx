...
interface GrocerySelfCheckoutPanelProps {
  language: Language;
  onSaved?: () => void;
}
...
export default function GrocerySelfCheckoutPanel({
  language: uiLanguage,
  onSaved,
  ...
  const persist = async (patch: Record<string, any>) => {
    try {
      ...
      rlog.error('[GrocerySelfCheckoutPanel] saveConfig failed:', err);
    }
  };
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          ...
          >
            <select
              value="retail_scan"
              disabled
              className="h-11 w-full rounded-lg border border-[var(--sand-300)] bg-slate-50 px-3 text-sm text-slate-600 outline-none"
            >
