...
export default function CartItemRow({
  ...
  return (
    <div className="bg-white border border-slate-200 shadow-sm rounded-lg p-3 mb-2.5 last:mb-0">
      ...
      <div className="flex items-center gap-2 mb-2.5">
        {onSetPrice && !editingPrice && (
          <button
            onClick={() => { setEditingPrice(true); setEditingNotes(false); }}
          ...
          >
            {tOr('pos.editPrice', 'Edit price')}
          </button>
        )}
        {onSetNotes && !editingNotes && (
          <button
            onClick={() => { setEditingNotes(true); setEditingPrice(false); }}
            className="h-11 px-3 text-xs text-slate-600 hover:text-brand-700 hover:bg-brand-50 rounded-md border border-slate-200 hover:border-brand-200 transition-colors cursor-pointer font-bold touch-manipulation"
          >
        ...
        <div className="inline-flex items-center rounded-lg border border-slate-300 bg-slate-50 overflow-hidden">
          ...
          {editingQty ? (
            <input
              ...
              className="w-14 h-11 text-center text-sm font-extrabold text-slate-950 bg-white border-x border-brand-300 outline-none"
              autoFocus
            />
          ) : (
            <button
              onClick={() => setEditingQty(true)}
              className="h-11 min-w-14 px-3 text-center text-sm font-extrabold text-slate-950 bg-white hover:text-brand-700 hover:bg-brand-50 cursor-pointer transition-colors border-x border-slate-300"
      ...
      {editingPrice && (
        <div className="mt-3 flex items-center gap-2">
          <input
            ...
            className="h-11 w-28 px-3 bg-white border border-slate-300 rounded-lg text-sm text-slate-950 font-bold text-right focus:outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-100"
            autoFocus
          />
          <button onClick={handleSavePrice} className="h-11 px-4 text-sm rounded-lg bg-emerald-600 text-white hover:bg-emerald-700 cursor-pointer font-bold">{tOr('pos.ok', 'OK')}</button>
          <button onClick={() => setEditingPrice(false)} className="h-11 px-4 text-sm rounded-lg bg-white border border-slate-300 text-slate-700 hover:bg-slate-50 cursor-pointer font-bold">{tOr('pos.cancel', 'Cancel')}</button>
        </div>
      )}
