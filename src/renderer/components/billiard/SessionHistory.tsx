...
export function SessionHistory({ language, onClose }: SessionHistoryProps) {
  ...
  return (
    <div className="h-full flex flex-col bg-slate-50 dark:bg-slate-900">
      ...
      {/* Filters bar */}
      <div className="px-4 py-2 bg-white dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-700/50 space-y-2">
        {/* Date range row */}
        <div className="flex items-center gap-2 flex-wrap">
          <button onClick={() => navigateDay(-1)} className="p-1.5 rounded-md hover:bg-slate-100 dark:hover:bg-slate-700">
          ...
          {(['today', 'yesterday', 'week', 'month'] as DateRange[]).map((r) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
                range === r ? 'bg-indigo-600 text-white' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700'
              }`}
            >
              {rangeLabels[r]}
            </button>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <select
            ...
            className="text-xs bg-white dark:bg-slate-700 border border-slate-300 dark:border-slate-600 rounded-md px-2 py-1.5 text-slate-700 dark:text-slate-200"
          >
          ...
          <div className="relative flex-1 max-w-[200px]">
            ...
            <input
              ...
              className="w-full pl-7 pr-2 py-1.5 text-xs rounded-md border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-800 dark:text-slate-200 placeholder:text-slate-400"
            />
          </div>
        </div>
      </div>
      ...
      {/* Pagination */}
      {totalPages > 1 && (
        <div className="flex items-center justify-between px-4 py-2 border-t border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800">
          <button
            onClick={() => setPage((p) => Math.max(0, p - 1))}
            disabled={page === 0}
            className="px-3 py-1.5 text-xs font-medium rounded-md border border-slate-300 dark:border-slate-600 hover:bg-slate-50 dark:hover:bg-slate-700 disabled:opacity-40"
          >
    ...
    <div className="bg-white dark:bg-slate-800/50">
      ...
      >
        ...
        <div className="min-w-[80px]">
          <p className="text-sm font-semibold text-slate-900 dark:text-white">{session.tableName}</p>
          {session.customer_name && (
            <p className="text-[10px] text-slate-500 dark:text-slate-400 truncate max-w-[100px]">{session.customer_name}</p>
          )}
        </div>

        {/* Time */}
        <div className="min-w-[100px]">
          <p className="text-xs text-slate-700 dark:text-slate-300">{formatDateTime(session.started_at, lang)}</p>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {session.payments.map((pay) => {
                  const Icon = METHOD_ICON[pay.method] || CreditCard;
                  return (
                    <div key={pay.id} className="flex items-center gap-1.5 px-2.5 py-1 bg-slate-50 dark:bg-slate-700 rounded-md">
                      <Icon className="w-3.5 h-3.5 text-slate-500" />
                      <span className="text-xs font-medium text-slate-700 dark:text-slate-300">{pay.method}</span>
                      <span className="text-xs font-bold text-slate-900 dark:text-white tabular-nums">{formatCurrency(pay.amount)}</span>
                    </div>
                  );
                })}
              </div>
              ...
              <div className="space-y-0.5">
                {session.items.map((item) => (
                  <div key={item.id} className="flex items-center gap-2 text-xs">
                    <span className="text-slate-700 dark:text-slate-300 flex-1 truncate">{item.name}</span>
                    <span className="text-slate-400 tabular-nums">x{item.quantity}</span>
                    <span className="text-slate-600 dark:text-slate-400 tabular-nums font-medium min-w-[60px] text-right">
