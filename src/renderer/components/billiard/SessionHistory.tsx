...
export function SessionHistory({ language, onClose }: SessionHistoryProps) {
  ...
  return (
    <div className="h-full flex flex-col bg-gray-50 dark:bg-gray-900">
      ...
      {/* Filters bar */}
      <div className="px-4 py-2 bg-white dark:bg-gray-800/50 border-b border-gray-100 dark:border-gray-700/50 space-y-2">
        {/* Date range row */}
        <div className="flex items-center gap-2 flex-wrap">
          <button onClick={() => navigateDay(-1)} className="p-1.5 rounded-md hover:bg-gray-100 dark:hover:bg-gray-700">
          ...
          {(['today', 'yesterday', 'week', 'month'] as DateRange[]).map((r) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
                range === r ? 'bg-indigo-600 text-white' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700'
              }`}
            >
              {rangeLabels[r]}
            </button>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <select
            ...
            className="text-xs bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md px-2 py-1.5 text-gray-700 dark:text-gray-200"
          >
          ...
          <div className="relative flex-1 max-w-[200px]">
            ...
            <input
              ...
              className="w-full pl-7 pr-2 py-1.5 text-xs rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-200 placeholder:text-gray-400"
            />
          </div>
        </div>
      </div>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between px-4 py-2 border-t border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800">
          <button
            ...
            disabled={page === 0}
            className="px-3 py-1.5 text-xs font-medium rounded-md border border-gray-300 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700 disabled:opacity-40"
          >
    ...
    <div className="bg-white dark:bg-gray-800/50">
      ...
      >
        ...
        <div className="min-w-[80px]">
          <p className="text-sm font-semibold text-gray-900 dark:text-white">{session.tableName}</p>
          {session.customer_name && (
            <p className="text-[10px] text-gray-500 dark:text-gray-400 truncate max-w-[100px]">{session.customer_name}</p>
          )}
        </div>

        {/* Time */}
        <div className="min-w-[100px]">
          <p className="text-xs text-gray-700 dark:text-gray-300">{formatDateTime(session.started_at, lang)}</p>
        ...
        <div className="flex items-center gap-0.5 shrink-0 min-w-[40px]">
          {session.payments.length > 0
            ? session.payments.map((pay) => {
                ...
                return <span key={pay.id} title={`${pay.method}: ${formatCurrency(pay.amount)}`}><Icon className="w-3.5 h-3.5 text-gray-400" /></span>;
              })
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {session.payments.map((pay) => {
                  const Icon = METHOD_ICON[pay.method] || CreditCard;
                  return (
                    <div key={pay.id} className="flex items-center gap-1.5 px-2.5 py-1 bg-gray-50 dark:bg-gray-700 rounded-md">
                      <Icon className="w-3.5 h-3.5 text-gray-500" />
                      <span className="text-xs font-medium text-gray-700 dark:text-gray-300">{pay.method}</span>
                      <span className="text-xs font-bold text-gray-900 dark:text-white tabular-nums">{formatCurrency(pay.amount)}</span>
                    </div>
                  );
                })}
              </div>
              ...
              <div className="space-y-0.5">
                {session.items.map((item) => (
                  <div key={item.id} className="flex items-center gap-2 text-xs">
                    <span className="text-gray-700 dark:text-gray-300 flex-1 truncate">{item.name}</span>
                    <span className="text-gray-400 tabular-nums">x{item.quantity}</span>
                    <span className="text-gray-600 dark:text-gray-400 tabular-nums font-medium min-w-[60px] text-right">
