      ...
      <div className="px-4 py-2 bg-white border-b border-slate-100">
        <div className="relative max-w-sm">
          ...
          <input
            ...
            className="w-full pl-8 pr-3 py-2 text-sm rounded-lg border border-slate-300 bg-white text-slate-800 placeholder:text-slate-400"
          />
        </div>
      </div>
            ...
            <tbody className="divide-y divide-slate-100">
              {filtered.map((row) => {
                ...
                return (
                  <tr
                    key={row.variant_id}
                    className={`transition-colors ${isLow ? 'bg-amber-50/50' : 'bg-white'} hover:bg-slate-50`}
                  >
                    {/* Name */}
                    <td className="px-4 py-2.5">
                      <span className="text-sm font-medium text-slate-900">{row.variant_name}</span>
                    </td>
                    ...
                    <td className="px-4 py-2.5 text-right">
                      {isEditing ? (
                        <input
                          ...
                          min="0"
                          step="1"
                          className="w-20 px-2 py-1 text-sm text-right rounded-md border border-teal-400 bg-white tabular-nums focus:outline-none focus:ring-1 focus:ring-teal-500"
                          autoFocus
                        />
                      ) : (
                        <span className={`text-sm font-bold tabular-nums ${isLow ? 'text-amber-600' : 'text-slate-900'}`}>
                          {row.quantity}
                        </span>
                      )}
                    </td>
                    ...
                    <td className="px-4 py-2.5 text-right">
                      {isEditing ? (
                        <input
                          ...
                          min="0"
                          step="1"
                          className="w-20 px-2 py-1 text-sm text-right rounded-md border border-slate-300 bg-white tabular-nums"
                        />
                      ) : (
