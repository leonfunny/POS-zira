...
export default function Settings({ config, onConfigChange, isModuleEntitled }: SettingsProps) {
  ...
  const [tvAdDevices, setTvAdDevices] = useState<TvDiscoveredDevice[]>([]);
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      ...
      const devices = await window.electronAPI.tvAdGetDevices().catch(() => []);
      if (alive) setTvAdDevices(Array.isArray(devices) ? devices : []);
    };
    tick();
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-lg border border-slate-200 bg-white p-3">
              <div className="mb-2 flex items-center justify-between gap-3">
                <div className="text-sm font-semibold text-slate-700">{t('settings.tvAd.onlineTvs')}</div>
                <span className="rounded-full bg-slate-100 px-2 py-0.5 text-xs font-medium text-slate-600">
                  {tvAdDevices.length}
                </span>
              </div>
              {tvAdDevices.length > 0 ? (
                <div className="space-y-2">
                  {tvAdDevices.map((device) => (
                    <div key={device.id} className="flex items-center gap-3 rounded-md border border-slate-100 bg-slate-50 px-3 py-2">
                      <span className="h-2.5 w-2.5 shrink-0 rounded-full bg-emerald-500 shadow-[0_0_0_3px_rgba(16,185,129,0.15)]" />
                      <div className="min-w-0 flex-1">
                        <div className="truncate text-sm font-medium text-slate-800">
                          {device.model || device.name || t('settings.tvAd.unknownTv')}
                        </div>
                        <div className="truncate text-xs text-slate-500">
                          {device.ip || t('settings.tvAd.unknownIp')}
                          {device.posHost ? ` · POS ${device.posHost}` : ''}
                        </div>
                      </div>
                    </div>
                  ))}
