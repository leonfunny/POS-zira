...
interface Dependencies {
  client: Pick<PosApiClient, 'getServerOrderDetail' | 'refundOrder'>;
...
function assertLegacyRefundOrder(database: AndroidDatabase, orderId: string): void {
  if (database.get('SELECT id FROM orders WHERE id = ? AND refund_event_context_json IS NOT NULL', [orderId])
    || database.get('SELECT request_id FROM pos_refund_events WHERE local_order_id = ? LIMIT 1', [orderId])) {
...
export function createAndroidRefundCoordinator(deps: Dependencies) {
  ...
  async function context() {
    ...
    return { salonId, scopeKey: JSON.stringify([serverUrl, salonId, userId]), assertContext };
  }
  function supportedOrder(database: AndroidDatabase, orderId: string, allowConfirmed = false) {
    assertLegacyRefundOrder(database, orderId);
  ...
  async function run(orderId: string, request: any, reconcileId?: string): Promise<any> {
    ...
    try {
      ...
      if (reconcileId && !attempt) throw new Error('Original refund attempt not found');
      if (attempt) {
        const saved = JSON.parse(attempt.expected_json);
        if (saved.protocolVersion !== undefined) throw new Error('Canonical refund event reconciliation is not enabled in this client yet');
      ...
      const { order, shift } = supportedOrder(database, orderId);
      ...
      if (!attempt) {
        ...
        attempt = repo.prepare({ request_id: requestId, scope_key: scope.scopeKey, local_order_id: orderId,
          backend_order_id: order.backend_id, shift_id: shift.id, payload_json: JSON.stringify(prepared.payload),
          expected_json: JSON.stringify({ authority: prepared.expected, priorRefundLines: prepared.priorRefundLines,
      ...
      const assertOriginal = async () => {
        await scope.assertContext();
        const now = supportedOrder(database, orderId);
        if (attempt!.backend_order_id !== now.order.backend_id || attempt!.shift_id !== now.shift.id
          ...
          throw new Error('Original refund order or shift changed; reconciliation requires the original accounting state');
        }
      };
  ...
  return {
    ...
    async getRefundDetail(orderId: string): Promise<any> {
      try {
        ...
        supportedOrder(database, orderId, true);
