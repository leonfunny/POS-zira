...
export function mergeRefundLines(
  ...
  for (const line of [...parseLocalRefundLines(existingRefundLinesJson), ...deltaRefundLines]) {
    ...
    const orderItemId = typeof line?.orderItemId === 'string' ? line.orderItemId.trim() : '';
    if (requestId && (lineKey || orderItemId)) {
      const identity = JSON.stringify([requestId, lineKey ? 'billiard' : 'item', lineKey || orderItemId]);
      if (seenRequests.has(identity)) continue;
...
export function toRefundBackendPayload(
  ...
  const hasExactLines = !hasBilliardLines && (data.lines ?? []).some(line => line.orderItemId !== undefined);
  if (hasExactLines) {
    const ids = new Set<string>();
    for (const line of data.lines ?? []) {
      const id = line.orderItemId;
      if (typeof id !== 'string' || !id.trim() || id !== id.trim() || id.length > 128 || ids.has(id)) {
        throw new Error('Refund lines must all use unique, valid orderItemId identity.');
      }
      ids.add(id);
    }
  }
