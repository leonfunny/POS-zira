...
import { calculateLineTotalGrosze, normalizeSaleUnit, normalizeSellBy, resolveSaleQuantity } from './pos-sale';
import { readRestaurantHistoryHeader, readRestaurantHistoryLines } from './restaurant-history-header';

export type OrderAdapterWarning = (field: string, sample: any) => void;
...
export function adaptServerOrder(s: any, warnOnce: OrderAdapterWarning = () => {}): any {
  ...
  const restaurantHeader = readRestaurantHistoryHeader(s);
  ...
  return {
    ...
    ...(restaurantHeader ? {
      table_id: restaurantHeader.tableId,
      covers: restaurantHeader.covers,
      order_type: restaurantHeader.orderType,
      _restaurantHeader: restaurantHeader,
    } : {}),
    sync_error: null,
    sync_attempts: 0,
...
export function adaptServerOrderItem(item: any, orderId: string, serverOrder?: any): any {
  const linkedLine = serverOrder?.id === orderId ? readRestaurantHistoryLines(serverOrder)?.get(item.id) : undefined;
  const restaurantLine = linkedLine?.productId === (item.variantId || item.productId) ? linkedLine : undefined;
  ...
  return {
    ...
    ...(restaurantLine && !billiardMetadata ? {
      restaurant_line_id: restaurantLine.localLineId,
      notes: restaurantLine.notes,
      course: restaurantLine.course,
    } : {}),
  };
}
